...
```{r}
# Wrangle data for lobster count year/site

lobster_count_year <- lobster_abundance %>% 
  group_by(year, site) %>% 
  summarize(total_count_site = sum(count, na.rm = TRUE)) %>% 
  mutate(MPA = case_when(
    site %in% c("IVEE", "NAPL") ~ "Protected Marine Area Site",
    site %in% c("AQUE", "MOHK", "CARP") ~ "Non-Marine Protected Site")) %>% 
...
ggplot(data = lobster_count_year, aes(x = year, y = total_count_site)) +
  geom_line(aes(color = MPA), 
                ...
                show.legend = TRUE) + 
  geom_point(size = 2) +
  scale_x_continuous(breaks = seq(2012, 2018, by = 2),
